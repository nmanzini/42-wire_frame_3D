...
/*   Updated: 2018/01/22 19:35:15 by nmanzini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		matrix_line(t_mlx_data *md, unsigned int color)
{
	...
	while (i < md->in->n)
	{
		...
		while (j < md->in->m)
		{
			if (i + 1 < md->in->n)
				line(md,
				md->in->matrix_p[j][i][0], md->in->matrix_p[j][i][1],
				md->in->matrix_p[j][i + 1][0], md->in->matrix_p[j][i + 1][1],
				md->in->line_color);
			if (j + 1 < md->in->m)
				line(md,
				md->in->matrix_p[j][i][0], md->in->matrix_p[j][i][1],
				md->in->matrix_p[j + 1][i][0], md->in->matrix_p[j + 1][i][1],
				md->in->line_color);
			j += 1;
		}
		i += 1;
